...
Cabinet-STRASSE は、ファイルマネージャーを入口にした資料キャビネットである。

まず、内部ストレージ、SDカード、USBストレージ、Download、Documents、Pictures などを開き、ファイルを探し、開き、コピーし、移動し、名前を変え、削除し、共有できることを基本価値とする。

その上で、VIASTRASSE ファミリーと連携し、ファイル、資料、添付、URL、成果物を整理、検索、参照、関連付けできるデジタル資産管理機能を重ねる。

単なるファイル一覧で終わらず、タグ、Collection、Smart Folder、Inbox、OCR、関連付けによって、ファイルを「あとで使える資料」として育てる。
...
Cabinet-STRASSE は、普通に使えるファイルマネージャーとして成立することを最初の条件とする。

ユーザーはまず「ファイルを開く」「場所を見る」「整理する」ために Cabinet を起動する。

その体験の上に、VIASTRASSE ファミリーの中で「資料の正本」を管理する役割を重ねる。

つまり Cabinet は、資料管理アプリからファイル操作へ降りるのではなく、ファイルマネージャーから資料活用へ伸びるアプリである。

```text
入口 = ファイルマネージャー
進化 = 資料キャビネット
連携 = VIASTRASSE ファミリーの正本管理
```
...
Cabinet-STRASSE は、他の STRASSE アプリがインストールされていなくても、通常のファイルマネージャーとして使える必要がある。

単体利用では、まずストレージとフォルダを扱えることが価値の中心である。
...
```text
普通のファイルマネージャーとして迷わず使える。
その上で、タグ、Collection、Smart Folder、Inbox、OCR、STRASSE連携によって資料キャビネットとして進化する。
```
...
画面上の重心は Explorer を先頭に置く。
Library、Collection、Smart Folder は Explorer の代替ではなく、ファイルを再利用しやすくする付加価値として扱う。
...
2. 最近使ったファイル
3. Inbox / お気に入り / ゴミ箱
4. Library / Collection / Smart Folder

これにより、ユーザーは Cabinet を「まずファイルを触るアプリ」として認識できる。
